...
class TestAuth:
    ...
    def test_raw_response_list(self, client: OpencodeSDK) -> None:
        response = client.auth.with_raw_response.list()
    ...
    def test_streaming_response_list(self, client: OpencodeSDK) -> None:
        with client.auth.with_streaming_response.list() as response:
...
class TestAsyncAuth:
    ...
    async def test_method_list(self, async_client: AsyncOpencodeSDK) -> None:
        auth = await async_client.auth.list()
        assert_matches_type(AuthListResponse, auth, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_raw_response_list(self, async_client: AsyncOpencodeSDK) -> None:
        response = await async_client.auth.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncOpencodeSDK) -> None:
        async with async_client.auth.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
